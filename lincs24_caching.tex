...
\begin{frame}{A bit of point process theory...}
	...
	Let $\mathcal{F}_t = \sigma(\Phi(s), s\leqslant t)$ be its \alert{internal history}.
\end{frame}
	...
	\begin{frame}{Two important distributions:}
		...
		\begin{center}
			...
			\begin{tikzpicture}
				...
				\draw[->, very thick, verdecito] (2,.3) -- node[midway, above, anchor=south] {$T_0\sim F_0$} (4,.3);
		
				\draw<2->[->, very thick, rojito] (0,.3) -- node[midway, above, anchor=south] {$T\sim F$} (-1.5,.3);
		...
		\begin{tabular}{m{4cm} m{6cm}}
			\alert{Inter-arrival distribution:} & 
					\begin{gather*}
						F_0(t) := P^0_\Phi (\tau_1 - \tau_0 \leqslant t), \quad
						E^0_\Phi[\tau_1] = 1/\lambda.
					\end{gather*} \\[-2em]
					\uncover<2->{
			\alert{Age distribution:} &
					\begin{equation*}
						F(t) := P (- \tau_0 \leqslant t) = \lambda \int_0^t 1-F(s)ds,
					\end{equation*}}
		\end{tabular}
...
\begin{frame}{Renewal processes}
	...
	\begin{itemize}
	\item Let now $\Phi$ be a \alert{stationary renewal process}, i.e. inter request times $\tau_{k+1}-\tau_k$ are $iid\sim F_0$.
	
	\item Assume that $F_0$ has a density, and define the \alert{hazard rate} of $F$ as:
	 \begin{equation*}
		\eta_0(t) = \frac{f_0(t)}{1-F_0(t)}
	 \end{equation*}
...
\begin{frame}{The Poisson case}
	...
	\pause \vfill
	\alert{Conclusion:} under Poisson arrivals, statically keeping the most popular objects is optimal (as in IRM).
\end{frame}
...
\begin{frame}{The Renewal case}
	\begin{myitem}[2em]
		...
		\item Since $\lambda_i(t) = \eta_i(t-\tau^-_i(t))$, the optimal policy is:
		\vspace{1em}
		\begin{itemize}
...
\begin{frame}{An interesting observation}
	...
	\begin{myitem}[1em]
		...
		\item If we haven't seen it for a while, we may have to fetch it \alert{anticipating} an upcoming request!
	\end{myitem}
\end{frame}
...
\begin{frame}{The threshold value in steady state}
	...
	At time $t=0$, we have a sample $\{X_1,\ldots,X_N\}$ of independent, but \textbf{not identically distributed} random variables, with distribution:
		...
		\begin{column}{0.5\textwidth}
			...
			\begin{tikzpicture}
				\begin{axis}[
					...
					xmin = -2.2,
					xmax=.5,
					xlabel style = {at={(axis cs:.5,0)},anchor=north west},
					y axis line style={draw=none},
					x axis line style={->, thick},
					ticks=none,
					axis x line*=middle,
				...
				\plot[domain=-2:0, very thick, color=azulcito]{2/(1+(x+2))} node[midway,anchor=south west] {\small $\eta_i(t-\tau_0^i)$};

				\draw[azulcito, very thick] (axis cs:-2,0) -- (axis cs:-2,2);
				\node[anchor=north] at (axis cs:-2,0) {\footnotesize $\tau_0^i$}; 
				\draw[->, violetita, very thick] (axis cs:0,0) -- (axis cs:0,2/3) node[anchor=west] {$X_i$};
				\node[anchor=north] at (axis cs:0,0) {\footnotesize $0$}; 
				\end{axis}
			\end{tikzpicture}
		\end{column}
	\end{columns}	
	...
	The threshold $\theta(0)$ is the $C-$th \alert{order statistic} (in decreasing order) of the sample.
	
	\vfill
	\pause

	\alert{Problem:} non $iid$ $\to$ no closed form $\to$ Can we say something about the large scale limit?
\end{frame}
...
\begin{frame}{A little more structure}
	...
	In this case:
	\begin{myitem}[1em]
		\item The distribution of $-\tau_0^i$ is $F^{(i)}(t) = F(\lambda_i t)$.
		\item The hazard-rate is $\eta_i(t) = \lambda_i \eta_0(t/\lambda_i)$.
...
\begin{frame}{The distribution of popularities}
	...
	Consider now the popularities $\lambda_1> \ldots > \lambda_N$ and define their empirical measure:
	\begin{equation*}
...
\begin{frame}{Main result}
	...
	\begin{theorem}[Carrasco,F',Paganini]
		Consider a caching system fed by $N$ independent and stationary renewal processes, with intensities $\{\lambda_i\}$, and inter-arrival distributions $F^{i}_0(t)=F_0(\lambda_i t)$. Let $X_1,\ldots,X_N$ denote the observed hazard-rates at time $0$. Then, under the preceding assumption, the empirical distribution:
		\begin{equation*}
...
\begin{frame}{Proof sketch}
	...
	\begin{myitem}[2em]
		...
		\item By Shorack's result:
		 ...
		 \item Note that:
		 \begin{equation*}
			\frac{1}{N} \sum_{i=1}^N G_i(x) = \sum_{i=1}^N G_0\left(\frac{x}{\lambda_i}\right) \frac{1}{N} = \int_0^\infty G_0\left(\frac{x}{\lambda}\right) \phi_N(d\lambda)
		 \end{equation*}

		 \item Use the assumption to show that:
		 \begin{equation*}
			\int_0^\infty G_0\left(\frac{x}{\lambda}\right) \phi_N(d\lambda) \to_M \int_0^\infty G_0\left(\frac{x}{\lambda}\right) \phi(d\lambda) = G_\infty(x).
		 \end{equation*}
...
\begin{frame}{Asymptotic miss probability}
	...
	\begin{theorem}
		...
		\begin{equation*}
			\lambda_{\text{miss},N} \to_N \int_0^\infty \lambda \tilde{G}_0\left(\frac{\theta^*}{\lambda}\right) \phi(d\lambda) = \E{\Lambda \tilde{G}_0\left(\frac{\theta^*}{\Lambda}\right)} 
		\end{equation*}
...
\begin{frame}{Choosing the optimal timers}
	...
	\begin{problem}[Optimal TTL policy]
		...
		\begin{equation*}
			\max_{T_i\geqslant 0} \sum_i \lambda_i F^{(i)}_0 (T_i)
		\end{equation*}
		subject to:
		\begin{equation*}
			\sum_i F^{(i)}(T_i) \leqslant C
		\end{equation*}
	\end{problem}
...
\begin{frame}{The optimal timers}
	...
	\begin{theorem}
		...
		\begin{itemize}
			\item \alert{Increasing hazard rate}: keep the most popular objects ($T_i=\infty$ or $0$).
			
			\item \alert{Decreasing hazard rate:}
...
\begin{frame}{Choosing the optimal timers}
	...
	\only<2->{
		\begin{problem}[Optimal pre-fetching policy]
			...
			subject to:
			\begin{equation*}
				\sum_i \hat{F}^{(i)}(T_i) \geqslant N-C
			\end{equation*}
		\end{problem}		
	}
	\vfill
...
\begin{frame}{Final remarks}
	...
	\begin{myitem}[2em]
		...
		\item Classical caching is not well-suited to regular traffic.
